...
void DFSUtil(int vertex, struct graph_data *graphData, int *visited,
  ...
  pthread_mutex_lock(&mutex);
  //if there are no unvisited neighbours then the node is a leaf node, add it to the result.
  if (n_threads == 0) 
  {
...
void BFSUtil(struct graph_data *graphData, int *visited,
  ...
  for (int i = 0; i < *nc; i++) {
    ...
    if (pthread_create(&threads[i], NULL, ProcessNodeThread, (void *)pnArgs) != 0) 
    {
      perror("pthread_create");
      exit(-4);
    }
